...
export const EtiquetaPDF = async (linhas: number[]) => {
  ...
  for (let index = 0; index < linhas.length; index++) {
    try {
      const response = await fetch(`/api/pagamentos/get/${linhas[index]}`);
      const data = await response.json();

      try {
        const response_saerj = await fetch(
          `/api/adicionais_saerj/get/${data.matricula_SAERJ}`
        );
        ...
        if (endereco == "" && linhaCidade == "") {
          toast.error(
            "Um ou mais associados selecionados não possui todos os dados para gerar etiqueta."
          );
          return false;
        }
